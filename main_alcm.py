...
def deljob(id):
    db_sess = db_session.create_session()
    job = db_sess.query(JobsC).filter(JobsC.id == id).first()
    if job and (current_user.id == 1 or job.team_leader == current_user.id):
        db_sess.delete(job)
        db_sess.commit()
    else:
        abort(404)
...
def adddep():
    ...
    if form.validate_on_submit():
        db_sess = db_session.create_session()
        if db_sess.query(DepartmentC).filter(DepartmentC.email == form.email.data).first():
            return render_template('add_dep.html', title='Добавление Департамента',
                                   form=form,
                                   message="Департамент с такой почтой уже есть")
        if db_sess.query(DepartmentC).filter(DepartmentC.title == form.title.data).first():
            return render_template('add_dep.html', title='Добавление Департамента',
                                   form=form,
                                   message="Департамент с таким именем уже есть")
        try:
...
def reddep(id):
    ...
    if not dep:
        ...
        if request.method == "GET":
            if current_user.id == 1 or dep.chief == current_user.id:
...
def deldep(id):
    db_sess = db_session.create_session()
    dep = db_sess.query(DepartmentC).filter(DepartmentC.id == id).first()
    if dep and (current_user.id == 1 or dep.chief == current_user.id):
        db_sess.delete(dep)
        db_sess.commit()
    else:
        abort(404)
